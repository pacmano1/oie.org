...
**Why the share card is light.** `og-image-2.jpg` is judged in a feed at thumbnail size, and the
...
**Why the file is called `og-image-2.jpg`, and why the page declares its dimensions.**
Platforms cache share cards on the image URL, and Discord has no purge tool, so replacing the
file at the same path leaves the old card in their cache indefinitely. The first attempt at this
was a query string, `og-image.jpg?v=2`, which busts the cache without touching the file. LinkedIn
then rendered the card as a badly upscaled ~400px derivative. The card itself was provably sharp:
1200 by 630, 4:4:4 chroma, identical quantization tables to the version before it, and LinkedInBot
fetched it with a 200. What the head was missing was `og:image:width`, `og:image:height` and
`og:image:type`, so nothing told LinkedIn the asset was large before it fetched, and it built
itself a small derivative. Those tags had always been absent; forcing the first real re-scrape in
months is what surfaced it. The fix declares the dimensions and drops the query string in favour
of a numbered filename, which removes both suspects at once and still gives Discord a URL its
proxy has never seen. Bump the number for the next card. The cost is that `deploy.sh` syncs the
root with `--delete`, so the previous filename starts returning 404.
