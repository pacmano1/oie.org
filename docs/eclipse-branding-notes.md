...
**Why the share card is dark when the site is light.** `og-image.jpg` competes in a feed at
thumbnail size, so it leads with the homepage h1 on `--color-headings` for contrast. The hero
has the opposite job: the visitor has already arrived and is about to read. The pink hero stays.
